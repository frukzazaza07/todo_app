...
try { //trickets_new
    ...
    $select = "SELECT trickets_new.id,
                      ...
                      trickets_assigned.assigned_to,
                      trickets_assigned.request_date,
                      CONCAT(user.fname,' (',user.nickname,')') as name
                      FROM trickets_assigned
                      RIGHT JOIN trickets_new ON trickets_assigned.tracking_new_id = trickets_new.id
                      RIGHT JOIN user ON trickets_assigned.assigned_to = user.id
                      WHERE trickets_assigned.id IN (
                      SELECT Max(trickets_assigned.id)
                      FROM trickets_assigned
                      GROUP BY trickets_assigned.tracking_new_id
