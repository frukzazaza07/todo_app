...
<body style="font-family: Sukhumvit Set; height: 100%; padding: 0px; margin: 0px;">
    ...
    <script>
        ...
        $.get("../component/aside.php", function(data) {
            ...
            $('#inprogressTracking').toggleClass('menu-active');
            $('#inprogressTracking > a').toggleClass('a-menu-active');
        });
        $.get("../component/head.php", function(data) {
            $('.user-container').append(data);
        });
        $.get('inprogress-tracking-engine/retive-user.php', function(res) {
        ...
        function retive_data() {
            ...
            $.ajax({
                url: 'inprogress-tracking-engine/select-inprogress-assigned.php',
                type: "post",
                data: {
                    json: jsonData
                },
        ...
        function chageTicket_status(id, ticket_id, staff_id, thisElem, title) {
            let ticket_status = $(thisElem).val();
            if (ticket_status == "assigned") {
                changeTo_assigned(id, ticket_id, staff_id, title);
            } else if (ticket_status != "") {
                var r = confirm("ต้องการ " + ticket_status + " ใบงาน " + ticket_id + " ?");
                if (r == true) {
                    if (ticket_status == "pendding") {
                        changeTo_pendding(id, ticket_id, staff_id);
                        retive_data()
                    } else if (ticket_status == "close") {
                        changeTo_close(id, ticket_id, staff_id);
                        retive_data()
                    }
                }
            }
        ...
        function changeTo_pendding(id, ticket_id, staff_id) {
            ...
            $.ajax({
                url: 'inprogress-tracking-engine/change-to-pendding.php',
                type: "post",
                data: {
                    json: jsonData
                },
        ...
        function changeTo_close(id, ticket_id, staff_id) {
            let JSONassinged_ticket = {
                assingedTo: <?php echo $_SESSION['id']; ?>,
                ticketNo: id,
                ticketId: ticket_id
            ...
            let jsonData = JSON.stringify(JSONassinged_ticket);
            $.ajax({
                url: 'inprogress-tracking-engine/change-to-close.php',
                type: "post",
                data: {
                    json: jsonData
                },
        ...
        function changeTo_assigned(id, ticket_id, staff_id, title) {
        ...
        $('#btn_assinged').click(function() {
            ...
            let jsonData = JSON.stringify(JSONassinged_ticket);
            $.ajax({
                url: 'inprogress-tracking-engine/insert-assigned.php',
                type: "post",
                data: {
                    json: jsonData
                },
        ...
        function autoComplete(data, success_status) {
            ...
            <td style="color: #0DDC4E">${data[i][0].request_status}</td> \
            <td>${data[i][0].name}</td> \
            <td>${data[i][0].request_date}</td>\
            <td>${data[i][1]}</td>\
                <td width="15%"> \
                <select class="form-control form-control-sm" onchange="chageTicket_status('${data[i][0].id}','${data[i][0].tracking_id}','${data[i][0].assigned_to}',this,'${data[i][0].request_title}')">\
