...
try {
    ...
    $insert = "INSERT INTO trickets_close(
                    tracking_new_id,
                    tracking_id,
                    assigned_to,
                    request_status) 
    ...
    if ($stmt_insert->execute()) {
        $answer["success"] = 1;
        $answer["message"] = "close Tickets: " . $JSONData['ticketId'] . " เรียบร้อย";
        exit(json_encode($answer));
    } else {
        $answer["success"] = 0;
        $answer["message"] = "ไม่สามารถ close Ticket: " . $JSONData['ticketId'] . " ได้ " . $PDO->errorInfo();
        exit(json_encode($answer));
    }
